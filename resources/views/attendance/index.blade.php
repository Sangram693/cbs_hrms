...
@section('content')
    <div class="bg-white p-6 rounded shadow">
        ...
        @if ($user->isSuperAdmin() || $user->isAdmin() || $isHr)
            <a href="{{ route('attendance.create') }}" class="mb-4 inline-block bg-blue-500 text-white px-4 py-2 rounded">Add
                Attendance</a>
        @endif
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($attendances as $attendance)
                        <tr class="hover:bg-gray-50">
                            @if ($user->isSuperAdmin() || $user->isAdmin() || $isHr)
                                <td class="py-3 px-6 text-center whitespace-nowrap">{{ $attendance->employee->name ?? '-' }}
                                </td>
                            @endif
                        ...
                        {{ $attendance->status === 'Present'
                            ...
                            @if ($user->isSuperAdmin())
                                <td class="py-3 px-6 text-center whitespace-nowrap">
                                    {{ $attendance->employee->company->name ?? '-' }}</td>
                            @endif
                            @if ($user->isSuperAdmin() || $user->isAdmin() || $isHr)
                                <td class="py-3 px-6 text-center whitespace-nowrap">
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ 6 + ($user->isSuperAdmin() ? 1 : 0) + ($user->isSuperAdmin() || $user->isAdmin() || $isHr ? 1 : 0) }}"
                                class="py-2 px-4 text-center border-t border-gray-300">No attendance records found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>

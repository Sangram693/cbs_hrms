    ...
    <form action="{{ route('profile.update') }}" method="POST">
        ...
        @if(!$isSuperadmin)
            ...
            <div class="mb-4">
                ...
                <input type="text" name="emp_id" class="w-full border rounded px-3 py-2 bg-gray-50" value="{{ old('emp_id', $employee->emp_id ?? '') }}" readonly>
            ...
            <div class="mb-4">
                ...
                <input type="text" class="w-full border rounded px-3 py-2 bg-gray-50" value="{{ $employee?->department?->name ?? 'Not Assigned' }}" readonly>
            ...
            <div class="mb-4">
                ...
                <input type="text" class="w-full border rounded px-3 py-2 bg-gray-50" value="{{ $employee?->position ? $employee->position->title . ' (' . $employee->position->level . ')' : 'Not Assigned' }}" readonly>
            ...
            <div class="mb-4">
                ...
                <input type="date" name="hire_date" class="w-full border rounded px-3 py-2 bg-gray-50" value="{{ old('hire_date', $employee->hire_date ?? '') }}" readonly>
                @error('hire_date')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
            </div>
            @if($employee)
            <div class="mb-4">
                <label class="block mb-1 font-semibold">Status</label>
                <input type="text" class="w-full border rounded px-3 py-2 bg-gray-50" value="{{ $employee->status ?? 'N/A' }}" readonly>
            </div>
            <div class="mb-4">
                <label class="block mb-1 font-semibold">Company</label>
                <input type="text" class="w-full border rounded px-3 py-2 bg-gray-50" value="{{ $employee->company->name ?? 'Not Assigned' }}" readonly>
            </div>
            @endif        @endif
        ...
        <div class="mb-6">
          ...
          <div class="flex justify-between items-center">
            <button type="submit" class="bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300">Update Profile</button>

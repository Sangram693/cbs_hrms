...
<div class="bg-white p-6 rounded shadow max-w-lg mx-auto">
    <h2 class="text-xl font-bold mb-4">Edit Login</h2>
    <form action="{{ route('logins.update', $login) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="employee_id" class="w-full border rounded px-3 py-2" required>
                ...
                @foreach($employees as $employee)
                    <option value="{{ $employee->id }}" {{ old('employee_id', $login->employee_id) == $employee->id ? 'selected' : '' }}>{{ $employee->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="username" class="w-full border rounded px-3 py-2" value="{{ old('username', $login->username) }}" required>
            @error('username')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
        </div>
        <div class="mb-4">
            <label class="block mb-1 font-semibold">Role</label>
            <input type="text" name="role" class="w-full border rounded px-3 py-2" value="{{ old('role', $login->role) }}" required>

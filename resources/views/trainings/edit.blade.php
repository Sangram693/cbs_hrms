...
<div class="bg-white p-6 rounded shadow max-w-lg mx-auto">
    ...
    <form action="{{ route('trainings.update', $training) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="date" name="start_date" class="w-full border rounded px-3 py-2" value="{{ old('start_date', $training->start_date) }}">
            @error('start_date')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
        </div>
        <div class="mb-4">
            <label class="block mb-1 font-semibold">End Date</label>
            <input type="date" name="end_date" class="w-full border rounded px-3 py-2" value="{{ old('end_date', $training->end_date) }}">

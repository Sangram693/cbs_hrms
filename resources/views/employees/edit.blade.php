        ...
        <form action="{{ route('employees.update', $employee) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="user_role" class="w-full border rounded px-3 py-2">
                    <option value="employee" {{ old('user_role', $employee->user_role ?? 'employee') == 'employee' ? 'selected' : '' }}>
                        Employee
                    </option>
                    @if (auth()->user()->isSuperAdmin())
                        <option value="admin" {{ old('user_role', $employee->user_role ?? '') == 'admin' ? 'selected' : '' }}>
                            Admin
                        </option>
                        <option value="super_admin" {{ old('user_role', $employee->user_role ?? '') == 'super_admin' ? 'selected' : '' }}>
                            Super Admin
                        </option>
                    @elseif(auth()->user()->isAdmin())
                        <option value="admin" {{ old('user_role', $employee->user_role ?? '') == 'admin' ? 'selected' : '' }}>
